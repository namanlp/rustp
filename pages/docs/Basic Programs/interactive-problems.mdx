...
description: In competitive programming contests, Interactive Problems might be seen quite often. We will discuss flushing the buffer for these problems here in Rust Language.
---
...
See  [Basic interactive problem on CodeChef](https://www.codechef.com/problems/CHFIDEAL) for reference.

## The `flush` Issue

Actually, the output to terminal is an *expensive* process for any programming language. If there are no buffers, each character will be shown as soon as output is generated. It will block the rest of the program thread, resulting in much slow execution of program.
...
But in the interactive problems, this is a major issue, because online judge will keep waiting indefinitely for the output, as it is stored in Buffer, and thus, no output will be generated, and the program keeps waiting for the output.
This will result in *deadlock* situation, and will result in **Time Limit Exceeded ( TLE )** verdict in most programming websites.

So, we have to print  the contents of buffer and clear its contents. This is called *flushing*.
