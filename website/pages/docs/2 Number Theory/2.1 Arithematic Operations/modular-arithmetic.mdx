...
order: 1
slug: /number-theory/modular-arithmetic/
...
Now, both **`i128`** and **`u128`** can store numbers upto order of **<code>10<sup>38</sup></code>**, which is pretty large, and both take 128 bits to represent.

But now suppose you have to tell the answer of **<sup>1000</sup>C <sub>500</sub>** , which is of order of **10<sup>299</sup>** . Clearly it is impossible to calculate using traditional computation methods. Even if it is possible in certain languages like python, it takes really long time to compute it and is certainly not suitable for competitive programming.

So, answers are judged on the basis of modulo of answer from a given a number, generally **<code>10<sup>9</sup>+7 or 1000000007</code>**. So, it is necessary to know the properties of Modular Arithmetic and their application in order to compute the results efficiently.
...
**Note :** There is no division property. As an alternative, See [Modular Multiplicative Inverse in Rust](/number-theory/modular-inverse/).
...
<SEO title="Modular Arithmetic - Rust Programming"
        description="Many times, the results become so large that it becomes impossible to store them in any numerical data structure. This article covers basics of Modular Arithmetic."
/>
