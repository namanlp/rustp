...
As the result is only dependent on amount, we can simply store the values in an array or a vector, at the index of amount itself.

For example, the result for amount = 15 will be stored at index 15.
